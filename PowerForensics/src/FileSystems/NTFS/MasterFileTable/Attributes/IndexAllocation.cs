...
namespace PowerForensics.Ntfs
{
    ...
    public class IndexAllocation : Attr
    {
        ...
        internal IndexAllocation(NonResident header, string volume)
        {
            ...
            for (int offset = 0; offset < bytes.Length; offset += 4096)
            {
                ...
                IndexBlock indexBlock = new IndexBlock(Util.GetSubArray(bytes, (uint)offset, (uint)indexBlockSize));

                // Create byte array for IndexEntry object
                // 0x18 represents the offset of the EntryOffset value, so it must be added on
                byte[] indexEntryBytes = Util.GetSubArray(bytes, (uint)offset + indexBlock.EntryOffset + 0x18, indexBlock.TotalEntrySize);
                ...
                do
                {
                    ...
                    IndexEntry indexEntry = new IndexEntry(Util.GetSubArray(indexEntryBytes, (uint)entryOffset, BitConverter.ToUInt16(indexEntryBytes, entryOffset + 0x08)));
    ...
    internal class IndexBlock
    {
        ...
        internal IndexBlock(byte[] bytes)
        {
            ...
            UpdateSequenceArray = Util.GetSubArray(bytes, (uint)(OffsetOfUS + 2), (uint)(2 * SizeOfUS) - 2);
        ...
        internal static void ApplyFixup(ref byte[] bytes, int offset)
        {
            ...
            if (ussize != 0)
            {
                ...
                byte[] UpdateSequenceArray = Util.GetSubArray(bytes, (uint)(usoffset + 2 + offset), (uint)(2 * ussize));
